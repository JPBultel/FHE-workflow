...
{
	bdt_pt result;
	std::cout << "something to be decrypted" << std::endl;
	cc->Decrypt(sk, tree.root, &(result.root));
	std::cout << "something decrypted" << std::endl;
	result.left = new bdt_pt();
	result.right = new bdt_pt();
	if(tree.left!=NULL)
	{
...
int main()
{
      ...
      //trivial de-serialization test
      std::vector<int64_t> vectorOfInts = {1};
      Plaintext test_plaintext = cc->MakePackedPlaintext(vectorOfInts);
      auto test_ciphertext = cc->Encrypt(pk, test_plaintext);
      
      if (!Serial::SerializeToFile(DATAFOLDER + "/" + "test_ciphertext.txt", test_ciphertext, SerType::BINARY)) {
        std::cerr << "Error writing serialization of test ciphertext to test_ciphertext1.txt" << std::endl;
      }
      std::cout << "The test ciphertext has been serialized." << std::endl;
      
      Ciphertext<DCRTPoly> new_test_ciphertext;
      if (Serial::DeserializeFromFile(DATAFOLDER + "/test_ciphertext.txt", new_test_ciphertext, SerType::BINARY) == false) {
        std::cerr << "Could not read the test ciphertext" << std::endl;
      }
      std::cout << "The test ciphertext has been deserialized." << std::endl;
      
      Plaintext new_test_plaintext;
      cc->Decrypt(sk, new_test_ciphertext, &new_test_plaintext);
      
      std::cout << "the de-serialized ciphertext has been decrypted." << std::endl;
      
      //de-serialization
      bdt_ct *p = new bdt_ct();
      ebdt_deserialize(p, "encrypted_tree", 3);
      bdt_ct new_encrypted_tree = *p;
      
      std::cout << "checking if the root exists" << std::endl;
      std::cout << "ciphertext at tree root :" << new_encrypted_tree.root << std::endl;
      ...
      Plaintext test_plaintext2;
      cc->Decrypt(sk, new_encrypted_tree.root, &test_plaintext2);
      std::cout << "root of the new tree decrypted" << std::endl;
      
      std::cout << "we are here" << std::endl;
      bdt_pt new_result = bdt_decrypt(cc, new_encrypted_tree, sk);
      
      //printing the result
      std::cout << "we are here" <<std::endl;
      std::cout << "first tree : " << result.root << (result.left)->root << (result.right)->root << (result.left)->left->root << (result.left)->right->root << (result.right)->left->root << (result.right)->right->root << std::endl;
      std::cout << "second tree : " << new_result.root << (new_result.left)->root << (new_result.right)->root << (new_result.left)->left->root << (new_result.left)->right->root << (new_result.right)->left->root << (new_result.right)->right->root << std::endl;
